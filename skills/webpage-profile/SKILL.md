...
Both dimensions are **mechanism** classifications: they live on one
`WebPresentation` System, never as Service props. The Service is `EXPOSED_VIA`
that System, which carries `rendering_model` and `navigation_model` as
**independent** props (each with its own confidence + evidence). There is no
per-rendering-mode System kind and no `RENDERED_BY` edge. You produce the two
classifications; the sole-writer places them.

## The two dimensions are INDEPENDENT (L1D-31a)

Classify each on its OWN signals - a fingerprint for one dimension says
...
A **SPA can be SSR-rendered** and an **MPA can use CSR widgets** - emit the two
dimensions separately and let each dimension's evidence stand on its own.

## A framework fingerprint alone is NEVER sufficient

A framework marker (`__NEXT_DATA__`, `id="root"`, `ng-version`, `data-reactroot`,
`window.__NUXT__`) names the *framework*, never the *behaviour*: Next.js ships
SSR, SSG, and CSR; Angular ships CSR and SSR. Where a fingerprint is the ONLY
...
interaction would settle the dimension - your probe becomes a backward-recon
request. Confidence discipline: **High** only with a behavioural signal; a
fingerprint alone caps you at **Low** with a probe; conflicting or thin signals
call for **Medium/Low + a probe** over a confident guess.
