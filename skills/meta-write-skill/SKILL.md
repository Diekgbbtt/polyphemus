...
Extend the skill the run just exercised: add the specific target detail it lacked, prune the content the run contradicted.
...
4. Rewrite through `write_skill`: `procedure` rewrites the whole SKILL.md, `references/<name>` writes one reference file; one whole file per call, written atomically; the bundle appears on first write.
