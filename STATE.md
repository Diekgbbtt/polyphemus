...
**Verification:** 1283 passed / 37 skipped (host, docker-down by design; the 5 docker-stack tests fail identically on base dev). E2e deliberately NOT run - the operator is addressing #95/#99 in another session before e2e.

**Deferred / deliberately NOT built:** no touch to StateGraph checkpoints / session / actor constructs (operator steer); compaction of the now-growing stateful context is `#95` (the `middleware` seam it plugs into is exposed; a long stateful run before compaction lands is a live, flagged risk); the `store` seam (`#85`) and the capability-adaptive inference-method config (`#99`) are seams only; the #73 escalating-timeout retry is NOT ported onto the session path (client per-turn retry only) - flagged robustness follow-up.
...
## AMENDMENT 2026-08-05 - #83: hunting agent (test-design side of Q8) + isolated e2e catalogue

Implementation `00498be` + review-fix follow-up `01ad689` merged into dev as `ac925cf` (merge: feat/hunting-83-hunting-agent into dev).

**What shipped.** `src/polymerhus/attack/hunting/hunting_agent.py` (build_hunting_agent seam: GROUND → SPEC-WRITE → pod loop → pure derive_verdict → D5 continuation judgment, fail-open at every seam; the ratified skill verbatims embedded ahead of every LLM turn), `hunting_tracing.py` (session-correlated Langfuse spans, fail-open), `skills/hunting/hunting-agent/SKILL.md`, `hunting` role in providers.ROLES, spec/impl docs, and the C1-C17 contract catalogue + the E3-E10 isolated e2e catalogue (real skill/store/join-key/tracing seams, fixture collaborators).

**The dead-code defect the e2e catalogue caught.** `_load_hunting_agent_skill` was never called anywhere - the ratified decision-tree verbatims never reached any LLM turn. Fixed by embedding the skill ahead of all three seams (authoring, re-authoring, judge); E3 pins the wiring.

**Code-review findings (separate /code-review pass, fixed in `01ad689`):** `_reenter` picked the superseded spec entry after an INIT re-author (now takes the LATEST committed experiment-log entry; E10 regression guard); `_MAX_RE_AUTHORING_PASSES` was dead code (now wired into the re-authoring guard); spec-composition trace span added at both authoring sites; ~150-line fixture duplication extracted into `tests/hunting_fixtures.py`; E9 degraded leg now uses the monkeypatch fixture.

**Verification (run FROM THE MAIN TREE against the live stack, per the operator's fast-forward requirement):** 72 hunting-tier tests + 195-test regression + data_modeller contracts all green; walkthrough E1-E2 stay declared-and-blocked on #84 (pod). The pre-existing D14 base defect (issue #96) is FIXED on dev (`7066e7c`, outside this ticket). **Independent loop-verifier APPROVED (2026-08-05)** - ran the suites itself, confirmed scope + no-cheating + the review-fix spot-checks; branch push + PR (`Closes #83`) authorised per issue-tracker.md.

## High Priority (loop is acting or waiting on human)

- [ ] **OPERATOR ACTION (before next app restart) — set `LLM_MODEL_HUNTING` in `.env` (2026-08-05).** #83 added `hunting` to `ROLES`, so `validate_llm_config` now requires `LLM_MODEL_HUNTING=<provider>:<model>` (+ its provider key) at boot; without it the app fails fast on startup ("LLM_MODEL_HUNTING must be set to '<provider>:<model>' (got None)"). Same pattern as the earlier `LLM_MODEL_ANALYSER` action (operator set `LLM_MODEL_ANALYSER=opencode:deepseek/deepseek-v4-flash-free`; an analogous `LLM_MODEL_HUNTING=opencode:...` fits). `.env.example` ships the sample. Tests mock the LLM so the suite is unaffected; only a real app boot needs the env var.
...
- [ ] **OPERATOR ACTION (2026-08-05) — set `LLM_MODEL_HUNTING` in `.env` before the next app boot.** #83 registered the `hunting` role; `validate_llm_config` hard-fails without it (same pattern as the earlier `LLM_MODEL_ANALYSER` action). `.env.example` now ships the sample line. Does NOT block the test suite or the verifier (LLM is mocked; the app is not booted by tests).
