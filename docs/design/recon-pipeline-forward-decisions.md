...
**D16 DISPATCH SUPERSEDED (#208, 2026-09-07) - the per-endpoint POD DISPATCH is replaced by ONE pod; the iteration set is unchanged.** The per-endpoint split paid **O(N) triager LLM turns** per reprofile run (each failing turn ~5min on a reasoning model before fail-open - the #206 amplifier; eval run `26bdaf55-...` starved analysis and the job was reaped `failed`). #208 collapses the dispatch: `httpx_reprofile` now dispatches **exactly ONE pod** regardless of endpoint count. `default_preprocess_fn` still runs `prepare_endpoint_profile_assets` (the dedup iteration set - dynamic-route collapse + root `/` materialisation + skip already-profiled non-root endpoints - is preserved verbatim) but packs the capped probe set into a single pod_input; the pod's configurator builds **one httpx exec over the full `-l` list** (`-o` JSON to the per-pod workdir + cat, the ffuf/arjun persistence pattern), so the phase pays **O(1) triager turns per job**. Production (exec->parse->curate of the profiles) and consumption (the triager's Observations) are structurally decoupled: a triager failure degrades to no-observations and never loses the parsed profiles (fail-open; #206's recovery turn, when it lands, makes the same turn converge). `MAX_JOB_ASSETS` now caps the probe SET (the whole set fits one pod); `MAX_PODS` no longer constrains the phase. Full record: `docs/design/per-endpoint-profiling-spec.md` (#208 supersession block).

**D16 CONSUMPTION-SET UNIFIED (#37, 2026-09-07, operator-chose option B) - one derivation for every job's input population; the route-cluster seam now serves TWO consumers.**
The defect: arjun `consumes="Endpoint"` with no selector was fed EVERY graph Endpoint, capped only by the blunt `MAX_JOB_ASSETS=500` truncation - daytona e2e run `3634e9e6-...`: ~500 endpoints -> ~40 min for the parameter-discovery pass (unbatched, one pod per endpoint, ~260 requests at `--rate-limit 5` each).
The fix unifies rather than branching: `default_preprocess_fn`'s accreted per-consumer branch chain (`batch` / `endpoint_profiling` / `api_scope` + the silent 1:1 fallback arjun fell through) is replaced by ONE composed derivation, `batching.derive_consumption_set`, driven by the per-job `JobSpec.consumption` options (malformed-path exclusion, route-cluster dedup, restapi-first ordering, pack into pod inputs).
The three working jobs are byte-identical by construction (equivalence tests pin the legacy building-block outputs as oracles): jsluice `pack="batches"`, httpx_reprofile `route_dedup + materialise_root + skip_profiled + pack="one_pod"`, kiterunner `pack="scan_targets"`; the legacy `batch` / `endpoint_profiling` / `api_scope` flags survive as read-only VIEWS over `consumption.pack` for the pod command builder and phase planner, which switch on dispatch shape, not input-set derivation.
The shared P1 seam holds: arjun rides `prepare_endpoint_profile_assets` with its grilled variant (`materialise_root=False` - probing `/` for parameters is low-value; `skip_profiled=False` - a content-type profile is orthogonal to parameter discovery), never a second ad-hoc dedup.
Arjun's ordering rule (P4: the cap is now a ceiling over a CHOSEN order, not a truncation): malformed-path exclusion via the curator gate's `is_malformed_concat_path` (called, never copied - P3), then `restapi`-profiled endpoints first, then the rest in stable input order - ordering only, never exclusion (corrected record 2: `webapp` is still probed).
The recorded exclusion set is exact: dedup + malformed-path + the inherited baseurl-less drop (assets with no derivable baseurl never survive the shared seam - near no-op for graph Endpoints, which always carry one).
Operator rulings from the #37 grilling, recorded so they are not re-derived: 404/403/401 responses are KEPT (a 404 on a crawled endpoint means the request shape is wrong - exactly where the job-specific agent must reason; Q6); katana `-aff` mangling artifacts (`/EXPRindex.php`, `/.json`) are NOT matched by the gate predicate and stay for the separate katana upstream ticket (this seam invents no URL-shape verdicts of its own); repeat-signature suppression is DEFERRED (no parameter-set plumbing - Q7); the #208 probe-vs-select boundary holds (this set feeds arjun; #208's request-shape adaptation classifies); #36 untouched (no blocking/backoff logic); `--rate-limit 5` unchanged.
Invariants preserved: auth threading end-to-end (P5, `use_auth` untouched), fail-open derivation (P6: a derivation failure degrades to capped raw 1:1 with a loud warning, never raises into the run).

**Forward - authN first-class (operator, #208 grilling, 2026-09-07).** The #208 grilling surfaced the auth direction behind the reprofile pass: auth context is to become a FIRST-CLASS system concern - per-endpoint auth shaping (cookies for a front-end surface, an `Authorization` header for a back-end surface, or one correctly-configured shape for all), an auth memory section (credentials, auth tokens scoped to an endpoint's base, an overview of the auth mechanism + fingerprinting), and a job-specific configurator-agent loop that questions its own auth context on 404/403/401 (what/how/why, validity, re-login for stale tokens, reshape for wrong headers). Structurally this will extend the authN threading and the job-specific agent's prompt, and the `httpx_reprofile` command_template's single `{auth_header}` becomes a more flexible, agent-shaped structure. NOT built in #208 - the one-pod shape keeps the single-auth threading exactly as today and is the seam the agent-shaped auth will later plug into. `parse_httpx` classification stays the loopback engine driver, unchanged.
