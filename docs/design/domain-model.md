...
A proposer's reasoning discipline arrives as a **skill** - a Markdown `SKILL.md` mounted as the role's system prompt - read through the **single skill loader** (`src/polymerhus/recon/domain/skills.py::skill_for`, FR-SKILLIF), which strips a machine-readable **data section** (`name`, `description`, `metadata.version`) every skill carries.
