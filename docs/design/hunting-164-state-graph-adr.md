...
The build scope is implemented on `feat/hunting-164-state-graph` (W1-W5 + follow-ups):

- W1 `hunter_state.py` (6fec8b5), W2 back-edge cut (32f30f7), W3 `hunter_memory.py` (ddcc6cd), W4 `hunter_tools.py` (aac741d), W5 StateGraph + turn-by-turn ReAct host (de0d4c4), SKILL sync (6b89e93), code-review findings fix (79d7f67), ADR + spec commit (368068d).
- `tests/attack/` 208 green; orchestrator contract/e2e green; the hypothesis lifecycle smoke runs end-to-end.
- **Deferred to the to-assertion phase (operator, 2026-08-23), now RESOLVED (operator, 2026-08-24)**: `tests/integration/test_hunting_agent_contracts.py` (C1-C17) and `tests/e2e/test_hunting_agent_isolated_e2e.py` (E3-E10) targeted the retired seam (`build_hunting_agent(*, store, kb, pod, author, judge)` no longer exists). They were **rewritten from scratch** per the to-assertions workbench and REPLACED by the memory assertion catalogue (`docs/design/hunting-164-assertion-catalogue.md`): `tests/integration/test_hunter_memory_contracts.py` (C1-C22, the store + tool seams) and `tests/e2e/test_hunter_memory_walkthrough.py` (E1-E4, the memory walkthroughs over the real harness + real store + scripted model), both GREEN on this branch.
- **The whole-hunter e2e verification (walking the workflow graph with the real LLM, the symbolic-layer materialisation, the full TestImplementationSpec lifecycle) is scoped AFTER the REST-capability workstream lands** (operator, 2026-08-24): that work exposes single hunting components and hunter/pod sessions at the app REST interface, so the hunter starts per project and consumes already-created `HuntConfig`s from the orchestrator. The walkthrough predicates are designed and accounted as validated in the catalogue (E5-E8), blocked on that workstream, and are iterated on this branch when it lands.
- **The spec-11 unit-tier seam tests are ALSO inside the R6 to-assertion deferral** (operator, 2026-08-23): the pure state machine, the compiled graph with `ainvoke`, the memory store, and the tool seams (`FakeLightRagTool` / `FakeMemoryStore` / `FakeExec`) all land with the to-assertion workbench, not with this branch. The memory system's integration contract tier (`test_hunter_memory_contracts.py`) covers the store + tool seams directly; the pure state machine / compiled-graph unit tier remains deferred.
- The ADR's earlier "the hypothesis lifecycle smoke runs end-to-end" claim is a manual smoke, not a committed test.
- No committed unit-tier test on this branch drives the hunter's `hunter_state` / `hunter_graph` seams (the only `ainvoke`-driven graph test in `tests/attack/` targets the orchestrator graph, `test_orchestrator_graph.py`).
