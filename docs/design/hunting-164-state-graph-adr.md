...
- **Deferred to the to-assertion phase (operator, 2026-08-23)**: `tests/integration/test_hunting_agent_contracts.py` (C1-C17) and `tests/e2e/test_hunting_agent_isolated_e2e.py` (E3-E10) still target the retired seam (`build_hunting_agent(*, store, kb, pod, author, judge)` no longer exists) and stay RED on the branch until the assertions workbench is re-authored against the new surface (verdicts -> `None` + pure `derive_verdict`, memory-store YAML, scripted models). The re-authoring is the R6-deferred workbench, not a fixture shim - a shim fabricating verdicts and hunt-store records would mask the retirement.
- **The spec-11 unit-tier seam tests are ALSO inside the R6 to-assertion deferral** (operator, 2026-08-23): the pure state machine, the compiled graph with `ainvoke`, the memory store, and the tool seams (`FakeLightRagTool` / `FakeMemoryStore` / `FakeExec`) all land with the to-assertion workbench, not with this branch.
- The ADR's earlier "the hypothesis lifecycle smoke runs end-to-end" claim is a manual smoke, not a committed test.
- No committed unit-tier test on this branch drives the hunter's `hunter_state` / `hunter_graph` / `hunter_memory` / `hunter_tools` seams (the only `ainvoke`-driven graph test in `tests/attack/` targets the orchestrator graph, `test_orchestrator_graph.py`).
