...
- **Streaming vs batch is not a substrate decision** `[L1D-23]`. Push (stream at recon time) or pull (post-recon batch) produce identical reads and writes. **Default batch for iteration 2**; add streaming only when the noise-reduction win is *measured*, not assumed. *(Streaming is the operator's original preference for reducing pattern-match noise; it is a two-way door.)* **Update 2026-07-17: the streaming door is now built and exercised as an opt-in (`settings.recon.streaming_analysis`) - see NM-7 below. Batch remains the default; the two-way door held (no substrate change). The pure-function contract's "identical reads and writes" prediction is confirmed live (idempotent convergence; final streamed pass == batch).**
...
- **NM-7 — Streaming analyser.** ~~Add the recon-triager→solution-analyser continuous stream once the noise-reduction win is measured; the pure-function contract makes push/pull interchangeable.~~ **IMPLEMENTED (opt-in) 2026-07-17** - operator-pulled into scope. `agent/recon/analysis/streaming.py::stream_analyser_step` + a per-job hook in `agent/recon/pipeline.py` (gated on `settings.recon.streaming_analysis`) feed each recon job's freshly-curated surface to the analyser DURING recon, so L1 is integrated progressively. Realised exactly as the pure-function contract predicts (repeated idempotent passes over the growing slice; final pass == batch). Batch stays the DEFAULT (L1D-23 two-way door). Verified: FR-STREAM ledger AST-STREAM-01..06 in `L1-MVP-plan.md` (unit + live, independently verifier-APPROVED). The measured "noise-reduction win" the original deferral asked for is NOT yet established - streaming did not reduce the static-asset over-assignment (that is L0-crawl + confidence-policy, AMV-8/AMV-9), and the analyser LLM shows high run-to-run assignment variance; those remain the open enhancement.
