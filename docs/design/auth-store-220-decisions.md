...
The bucket is `data/<project_id>/auth/` under `AUTH_STORE_ROOT` (`src/polymerhus/app/auth/data/`, no env var; explicit-root constructor for tests), lazily created at the first write.
