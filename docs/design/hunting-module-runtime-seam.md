...
- The **hunting module context**: the SHARED `module_context("hunting")` seam (`app/llm/checkpoints.py`), set by `hunting_module_context()` at the module's lifecycle entry points - never a private hunting-only var. `copy_context` propagation carries it into executor work, so work offloaded from hunting code resolves the hunting checkpointer automatically. Pool routing is gone - there is one shared executor.
...
- `POST /projects/{project_id}/hunting` - launch a hunting run (returns `hunting_run_id`). Refused 409 while the project holds a live `running` run (one live run per project, API-tier guard read before the row opens); the run row is the server-side at-most-once marker and any post-open refusal closes it to `failed` (T5, tracker #174).
...
- **Singular component launches (T5, added by ADR #169 Q17 / spec #169 "REST surface")**: `POST /projects/{project_id}/hunting/hunt` and `/hunting/pod` (202) enqueue ONE produced hunt config / test spec into the project's handoff families so the run-scoped inbox surfer dispatches one hunter / pod on the next tick; `/hunting/orchestrator` (202) schedules ONE orchestrator pass (no surfer, no run row - the produced/consumed topology has no orchestrator input family, so the pass IS the orchestrator's enqueue). A singular launch never fabricates a chained-dependency error; a replayed enqueue of the same identity is refused 409 by the storage novelty gate.
- **Per-session lifecycle verbs (T5, ADR #169 Q17)**: `POST /projects/{project_id}/hunting/{hunting_run_id}/sessions/{session_id}/pause|resume|stop` route ONE registered session to the shared runtime's per-session `hold_session` / `resume_session` / `cancel_run` (module `hunting`, keyed by the ADR Q13 session id): 404 unknown run / unknown-unregistered session, 503 no active runtime.
- **Surface ownership**: the handlers land in the existing `project_management/api.py` HTTP adapter router, added by the module runtime wiring (#110) as an additive block AFTER the runtime independence workstream's PRs land on `dev` (sequenced landings; the runtime-independence workstream lands first - one `workflow` ticket per layer: checkpoints refactor, session-address helper, runtime manager + lifecycle state machine + feed + per-module gate, app wiring, hunting control-plane machinery + `hunting_runs` DDL/accessors/reconcile, resume-seam scaffold + address audit - then #110 rebases). My PRs deliver the harness those handlers call: `runtime.schedule("hunting", ...)`/`runtime.cancel_run("hunting", ...)` marshalling and the `hunting_runs` DDL + accessors + startup orphan reconcile in `app/clients/pg.py`. Neither PR writes a file the other PR also writes.
