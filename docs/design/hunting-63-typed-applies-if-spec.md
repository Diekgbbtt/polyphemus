...
- **Witness.** A deterministic `does-not-apply` carries the violated clause id. The witness rides the `FaultSource` output's `applies-witnesses` slot (Q2 signature `f(L1) -> {(unit, fault, symptom, applies-witnesses)}`), fault-agnostic on the wire (the fault joins via `correlation_id` in the hunt store, Q6), and persists with the candidate state (#68).
...
**Seams under assertion:** the deterministic stage `evaluate(predicate, unit_projection) -> {pass, does-not-apply, witness}` (pure, injectable `read_fn`); the authoring-time predicate validator; the unit projection reader (index-card + one-hop both directions). Mechanised when #71's engine and #66's hardened entries exist.

## Contract predicates (integration)

- **C1 - Determinism.** Seam: deterministic stage, projection reader. Delivery semantic: success. Input: one fixed (predicate, unit projection) pair evaluated twice. Observable: both runs yield the identical verdict and the identical witness; zero calls to any LLM collaborator. Yields: integration test evaluating the pure function twice over a fake `read_fn`.
- **C2 - Necessary-only FALSE.** Seam: deterministic stage. Delivery semantic: success. Input: a predicate whose every clause is a typed-facet violation class, evaluated over a projection where exactly one clause is FALSE. Observable: verdict `does-not-apply`; the witness names exactly the violated clause; no clause produced FALSE from an absent facet or an NL operand. Yields: integration test asserting FALSE implies a clause-level typed-facet contradiction.
- **C3 - Default-open on unknown.** Seam: deterministic stage. Delivery semantic: empty-valid / absence. Input: a projection missing the facet a clause ranges over (e.g. `exposure` absent), and a projection whose facet value is unvalidated. Observable: both evaluate `pass`; UNKNOWN never prunes; the verdict is `pass`, not `does-not-apply`. Yields: integration test over absent-attribute and unvalidated-value projections.
- **C4 - Grammar validation (hard reject).** Seam: authoring-time validator. Delivery semantic: malformed. Input: a predicate artifact with an unsupported clause form, a value clause over an unvalidated facet (e.g. `spine-equals(rendering_model, "CSR")` before D2), and a non-AND composition. Observable: every input is rejected at validation; none reaches evaluation; the stage state does not change. Yields: integration test asserting hard-reject for each malformed class.
- **C5 - S1 subsumption parity.** Seam: deterministic stage. Delivery semantic: ordering / regression. Input: the set of (unit, fault) pairs the Q8 S1 spine-existence check would prune, for faults whose predicate is hardened. Observable: every such pair evaluates `does-not-apply` under the predicate (the predicate's prune set is a superset of S1's - no regression in what the deterministic gate prunes). Yields: integration test over a fixture L1 model comparing the two prune sets.
- **C6 - Fail-open.** Seam: deterministic stage. Delivery semantic: degradation. Input: a projection reader that raises and a malformed-at-runtime projection. Observable: verdict `pass` with a logged diagnostic; no exception escapes the stage; no pair is pruned by the failure. Yields: integration test with a raising fake `read_fn`.
- **C7 - Witness fault-agnosticism.** Seam: deterministic stage -> hunt store. Delivery semantic: success. Input: a deterministic `does-not-apply` for two different faults on the same unit. Observable: each witness records the violated clause id; neither witness carries the other fault's identity; both join by `correlation_id` only (the Q6 rule). Yields: integration test over the witness record shape.
- **C8 - Per-entry degrade.** Seam: deterministic stage, KB artifact. Delivery semantic: degradation. Input: one hardened entry (typed predicate present), one unhardened entry (enum tag only), one untagged entry. Observable: the hardened entry never consults its tag (R-c retired for it); the unhardened entry's tag is the deterministic signal; the untagged entry prunes nothing - each evaluated over the same unit set. Yields: integration test over the three entry classes.

## Walkthrough predicates (end-to-end)

- **E1 - A hardened fault prunes only the structurally impossible, and the LLM match sees only passers.** Grounds: user stories 1, 2, 7. Entry seam: the `FaultSource` selection entry, walking-skeleton L1 model. Input: the fault class "GraphQL introspection" carrying the typed predicate `target: Both; AND(reachable-via(EXPOSED_VIA, {GraphQLApi}))`, over a project whose L1 has one GraphQLApi System reached by services S1, S2 and one RESTApi-only service S3. Live edge: none (deterministic stage; the LLM match is stubbed as the pass-through seam for this walkthrough). Path: S1 and S2 evaluate TRUE on the clause (each `EXPOSED_VIA` a GraphQLApi); S3 evaluates FALSE (`EXPOSED_VIA` only RESTApi); the stage emits `does-not-apply` with the `reachable-via` witness for (S3, fault); S1 and S2 pass. Terminal: exactly 2 of 3 services pass; exactly 1 deterministic `does-not-apply` recorded, witness clause id `reachable-via(EXPOSED_VIA, {GraphQLApi})`; 0 pairs reach the LLM match beyond the 2 passers. Observed: the stage's evaluation log read back, listing 3 evaluations, 2 pass, 1 `does-not-apply`. Yields: e2e walkthrough over a seeded live L1 model.
- **E2 - Unknown-facet default-open in the live model.** Grounds: user story 6; C3 in the live tier. Entry seam: same. Input: a hardened fault whose predicate clause is `spine-present(exposure)` over a project where service S4 carries `exposure`, S5 omits it (not-yet-filled convention). Observable: S5 evaluates `pass` (UNKNOWN, never prune); both S4 and S5 reach the LLM match. Terminal: 2 of 2 pass, 0 pruned, 0 witnesses. Yields: e2e walkthrough.
- **E3 - Unhardened entry degrade in the live model.** Grounds: user story 6; C8 in the live tier. Entry seam: same. Input: an unhardened fault entry carrying only `enum-of-system-kinds: {WAF}` over a project with one WAF-fronted service S6 and one non-fronted service S7. Observable: S6 passes the tag signal; S7 is pruned by the tag; the typed stage is inert for the entry. Terminal: 1 pass, 1 pruned by the enum tag, 0 typed predicates evaluated. Yields: e2e walkthrough.
- **E4 - System-strict faults mint no Service candidates.** Grounds: user story 8; D-B. Entry seam: candidate-minting (consumes the predicate's `target` declaration). Input: a fault with `target: System` (e.g. a WAF-bypass class). Observable: no Service candidate is minted for the fault in the candidate set; the System candidates are evaluated. Terminal: 0 Service candidates for the fault; N System candidates exactly equal to the project's WAF-kind Systems; no `does-not-apply` emitted for services. Yields: e2e walkthrough over the skeleton's candidate set.
- **E5 - S1-subsumption parity live.** Grounds: D-B; C5 in the live tier. Entry seam: same. Input: the walking-skeleton L1 model and the skeleton's fault set with hardened predicates. Observable: the set of (unit, fault) pairs the deterministic stage prunes is a superset of the set the Q8 S1 spine-existence check would prune on the same model. Terminal: quantified by the two prune-set sizes; the S1-only pruned count is zero (every S1 prune is reproduced by a predicate clause). Yields: e2e walkthrough comparing the two deterministic filters over the live model.
