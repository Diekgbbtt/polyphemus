...
observable: the store holds exactly ONE note keyed by the spec id + variant order 0 (`kind == experiment_summary`, `order == 0`, key prefixed `<spec_id>:0:`), body mentions the 404; the triager's production turn terminates `{unsuccessful, space-exhausted}`.
...
terminal: `{unsuccessful, space-exhausted}`, `clean True`, exactly 1 raw observation status 404; the store holds exactly ONE note (`kind experiment_summary`, `order 0`, key prefixed by the #164 `<fault>_<strategy>` spec id, body mentions "404"); the triager's production interpretation note carries "third-party miner".
