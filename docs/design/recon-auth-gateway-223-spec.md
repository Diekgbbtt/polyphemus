...
The session proxy is disabled by default and this spec puts proxy support out of scope (#196), so nothing in the current design pins an egress; the store holds no network-identity fact and the profile key cannot express one.
...
The browser path additionally carries no humanization (`--stealth`), no interaction-cadence discipline, no challenge protocol, and a 10-minute session cap that expires while a human solves, so a low score has no recovery.
Remediation candidates (not decided here): pin a fixed or dedicated egress and persist its reference beside the profile key; prefer mount-and-verify over a fresh login whenever a mounted profile verifies; enable humanization on the browser path; and define a challenge protocol that detects the challenge's own marker and escalates.
Owner: the external bootstrap revision (#237) for the skill and prompt half, and a follow-up ticket for the network-identity half, since #196 owns proxies.
