...
flowchart TB
    ...
    subgraph LIVE["the one live skill-file → LLM-prompt binding"]
        ...
        F2["steel-crawl/SKILL.md"] -->|_load_skill via skill_for| CRA["crawler system prompt"]
    end
...
The pod's **`configurator`** node builds the command string. For a normal job it calls `fill_template`, substituting `{target}`/`{domain}`/`{baseurl}`/`{session}` from the input asset and `{auth_flags}` from the auth feed's per-phase projection when the job is `use_auth` (the pipeline resolves the gateway-bound account identifier from the shared store at assembly and threads the flat request material as `extra["auth_context"]`, serialised per tool at fill time by `control/auth_feed.py::serialize_auth_flags`). The serialiser speaks each tool's header dialect: the default repeated `-H "k: v"` flags for httpx/katana/ffuf/kiterunner, arjun's single newline-joined `--headers` blob, and graphql-cop's comma-joined `Key:Value` `--headers` argument — every value `shlex`-quoted so an operator token can never break the command. (Supersedes the retired settings-blob `{auth_header}` path, #223 T4 #243.) A batched job (`jsluice`) skips template-fill entirely and builds its command from the bundle batch via `batching.build_batch_command`.
...
A `project` has **one** `settings` row (PK/FK on `project_id`, `ON DELETE CASCADE`) whose `recon` JSONB blob holds `target_domain` and scope (settings carry no auth since #223 T4 #243 - auth lives in the shared store, seeded via `PUT /projects/{id}/auth`). Settings are written by `save_settings` with a **recursive `jsonb_deep_merge`** (a custom SQL function) rather than a plain `||`. The reason is correctness of partial PUTs: a PUT that only sets `scope.mode` must not wipe a previously-stored `scope.exclusions`, and a PUT that only adds `scope` must not drop `target_domain` (which would silently fall back to the `example.com` placeholder). `jsonb_deep_merge` descends into nested objects; scalars and arrays (an exclusions list) are still replaced wholesale, so setting exclusions overwrites the list as expected.
