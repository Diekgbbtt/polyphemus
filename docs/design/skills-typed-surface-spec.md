...
6. As an agent owner binding tools, I want one `load_skill(name)` tool with a verbatim contract, so that every stateful agent offers identical skill-loading semantics.
...
- The L1 index middleware, the `context` turn seam, and the hunter `system_prompt=` conversion are built (Q3); binding agents (middleware + `load_skill` + bounded `skills` context) is each agent owner's ticket, not this spec's.
