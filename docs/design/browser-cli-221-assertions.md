...
Command shapes below are the 2026-09-16 captures: where a variadic verb appears without its `--`, the capture predates D18 and is evidence, not a canonical encoding.
...
- Tool loop with no live session: before D18 the same mis-ordered command executed byte-identical through `steel_exec` (stubbed binary, no argument normalization); after D18 it is `refused:variadic-boundary` with nothing executed.
