...
- **Catalogue.** Shared skills stay flat under the repo's top-level `skills/`; both meta-skills were flat entries there, since REPLACED by the `skills/meta/` taxonomy (`is_meta_skill`, path-keyed exemption - D234-17). The per-project bundle is `<codebase_root>/data/<project_id>/skills/<skill_name>/` in the canonical layout (`SKILL.md`, `references/`, `scripts/`, `assets/`). There is no canonical `auth_workflow` skill; a project's copy is its original.
