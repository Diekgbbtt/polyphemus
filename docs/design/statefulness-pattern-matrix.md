...
| Dimension | Values |
|---|---|
| **Execution model** | `sync leaf` = called synchronously from a graph node or function; `async leaf` = a StateGraph node whose seam is an `async def` awaited through `_await_seam` (async awaited natively, sync offloaded via `asyncio.to_thread`); `async actor` = persistent `run_session_agent` loop with `AgentInbox`; `StateGraph` = LangGraph `StateGraph` compiled and invoked |
...
| `pod_runner` (test-executor runner) | StateGraph node (async leaf) | checkpointer (create_agent) | `default_runner_step_fn` -> `arun_session_turn("pod_runner", HuntSession(...), new_messages=delta, tools=[exec, note] (+ the config-gated `query_lightrag`/`KbQueryTool`), middleware=[compaction, harness], system_prompt=P0-P3)` - ONE ReAct stretch per `runner_agent` node turn (D84-16/17/22/29; as of T7) | `HuntSession(run_id, hunt_id, spec_id, "pod_runner")` -> `run:{hunt_id}:{spec_id}:pod_runner` (D84-34; the #164 `<fault>_<strategy>` spec id, NOT a root-spec hash); the memory store keys on the same spec id (D84-34) | `agents.py` (seam); `graph.py` runner_agent node; `harness.py` |
| `pod_triager` (test-executor triager) | StateGraph node (async leaf) | checkpointer (create_agent) | `default_triager_fn` -> `stateful_turn("pod_triager", HuntSession(...), schema=TriagerDecision via ToolStrategy, system_prompt=miner)` reading the verbatim P3 note + filtered context (D84-23/29; as of T7) | `HuntSession(run_id, hunt_id, spec_id, "pod_triager")` -> `run:{hunt_id}:{spec_id}:pod_triager` (D84-34; the #164 `<fault>_<strategy>` spec id) | `agents.py` (seam); `graph.py` triager node |
| `build_actor_hunting_agent` -> `build_hunting_agent` dispatch seam | async harness (dispatch_fn) | turn-by-turn ReAct host + per-project `HunterMemoryStore` (#164 W5) | `dispatch_fn(config)` drives `arun_session_turn("hunting_hunter", HuntSession(...), response_format=ToolStrategy(HunterStep), tools=build_hunter_tools(...))` step-by-step over the state-graph hunter | `HuntSession(run_id, hunt_id)` thread; the `HunterMemoryStore` keys on `<fault>_<strategy>` | `llm.py` (builder); `hunting_agent.py` (W5 harness); `hunter_graph.py` (state tracker) |
...
**Pod-role registration status (as of this matrix):** `pod_runner` / `pod_triager` join `HUNTING_ROLES` as `session`/`high` with their own `LLM_MODEL_POD_*` keys (D84-1); the two rows above record the RATIFIED execution shape (D84-2/7/12/23/29). The stateful session wiring landed with the #84 regrounding (T2-T7): the `HuntSession` address derivation (`pod/llm.py`), the `build_role_compaction_middleware` wiring, and the T7 production default seams (`agents.py` ReAct runner + note-reading triager) - these rows are observed facts as of T7 (#157). The contract tier still injects sync fakes (`runner_step_fn`/`triager_fn`) and keeps the old `runner_agent` <-> `tool_exec` bounded loop, LLM-free.
