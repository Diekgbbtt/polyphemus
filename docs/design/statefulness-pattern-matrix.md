...
| `pod_runner` (test-executor runner) | StateGraph node (async leaf) | checkpointer (create_agent) | `default_runner_step_fn` -> `arun_session_turn("pod_runner", HuntSession(...), new_messages=delta, tools=[exec, kb_retrieve, note], middleware=[compaction, harness], system_prompt=P0-P3)` - ONE ReAct stretch per `runner_agent` node turn (D84-16/17/22/29; as of T7) | `HuntSession(run_id, hunt_id, root_spec_hash, "pod_runner")` -> `run:{hunt_id}:{root_spec_hash}:pod_runner` (D84-2); the memory store keys on the same root spec hash (D84-20) | `agents.py` (seam); `graph.py` runner_agent node; `harness.py` |
| `pod_triager` (test-executor triager) | StateGraph node (async leaf) | checkpointer (create_agent) | `default_triager_fn` -> `stateful_turn("pod_triager", HuntSession(...), schema=TriagerDecision via ToolStrategy, system_prompt=miner)` reading the verbatim P3 note + filtered context (D84-23/29; as of T7) | `HuntSession(run_id, hunt_id, root_spec_hash, "pod_triager")` -> `run:{hunt_id}:{root_spec_hash}:pod_triager` (D84-2) | `agents.py` (seam); `graph.py` triager node |
