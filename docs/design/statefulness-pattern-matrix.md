...
| `pod_runner` (test-executor pod) | sync leaf | ContextVar + stateful_turn (T7) | `pod_session()` -> `stateful_turn` or `chat_model_for` | `HuntSession(run_id, hunt_id, spec_hash, role)` via ContextVar (D84-7; run_id/hunt_id derived from the parent `hunt_session`) | `pod/llm.py:53` |
| `pod_triager` (test-executor pod) | sync leaf | ContextVar + stateful_turn (T7) | `pod_session()` -> `stateful_turn` or `chat_model_for` | `HuntSession(run_id, hunt_id, spec_hash, role)` via ContextVar (D84-7; parent-derived, task-local default outside a hunt) | `pod/llm.py:53` |
