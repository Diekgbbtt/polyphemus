...
**Pod-role registration status (as of this matrix):** `pod_runner` / `pod_triager` join `HUNTING_ROLES` as `session`/`high` with their own `LLM_POD_*` keys (D84-1); the two rows above record the RATIFIED execution shape (D84-2/7/12/23/29). The stateful session wiring landed with the #84 regrounding (T2-T7): the `HuntSession` address derivation (`pod/llm.py`), the `build_role_compaction_middleware` wiring, and the T7 production default seams (`agents.py` ReAct runner + note-reading triager) - these rows are observed facts as of T7 (#157). The contract tier still injects sync fakes (`runner_step_fn`/`triager_fn`) and keeps the old `runner_agent` <-> `tool_exec` bounded loop, LLM-free.
