...
def test_runner_delta_lists_the_on_file_notes(log, store, spec_id):
    store.append(spec_id, variant_ref="v1", note_name="capability", kind="kb_insight",
                 body="payload family X")
    delta = compose_runner_delta(log, SPEC, "", 1, 8, store=store, spec_id=spec_id)
    assert f"{spec_id}:v1:capability" in delta
...
def test_triager_delta_reads_the_verbatim_p3_note(log, store, spec_id):
    store.append(spec_id, variant_ref="v1", note_name="experiment",
                 kind="experiment_summary", body=_NOTE_BODY)
    obs = RawObservation(status=404, body="not found")
    delta = compose_triager_delta(log, SPEC, obs, store=store, spec_id=spec_id, variant_ref="v1")
...
def test_triager_delta_falls_back_when_no_consolidation_note_yet(log, store, spec_id):
    delta = compose_triager_delta(log, SPEC, None, store=store, spec_id=spec_id, variant_ref="v0")
...
def test_triager_delta_is_fail_open_without_a_store(log, spec_id):
    delta = compose_triager_delta(log, SPEC, None, store=None, spec_id=spec_id,
                                  variant_ref="v0")
...
def test_memory_guidance_is_the_indexable_list_plus_the_reading_contract(store, spec_id):
    store.append(spec_id, variant_ref="v0", note_name="capability", kind="kb_insight",
                 body="x")
    guidance = compose_memory_guidance(store, spec_id)
    assert f"{spec_id}:v0:capability" in guidance
