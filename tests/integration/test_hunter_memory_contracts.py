...
def test_C8_read_specs_listing_sides_and_statuses(tmp_path):
    ...
    # a missing fault_key is a valid empty result, never a failure
    assert store.read_specs(PROJECT, "Service:slug:b|fault-y") == []
...
def test_C14_notes_read_grep_match_and_read_latest(tmp_path):
    ...
    store.write_note(PROJECT, action="append", fault_key="Service:slug:b|fault-y",
