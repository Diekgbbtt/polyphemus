...
// The two independent layer switches the graph toggles drive. Each layer is
// simply on or off; the four combinations need no extra vocabulary:
//   l0 + l1 - the full graph (every node, every edge)
//   l0 only - the descriptive attack surface (L0 nodes + L0-L0 edges)
//   l1 only - the service/system model: every L1 node, plus only the L0 nodes it
//             anchors to through a cross-layer edge (see CROSS_LAYER_EDGES). An
//             L1 node's anchors are its evidence, so they travel with it.
//   neither - the empty graph
export type LayerVisibility = { l0: boolean; l1: boolean }
...
// With L0 switched off these are the ONLY reason an L0 node is shown.
...
// Stable identity for a layer combination. GraphCanvas remounts ForceGraph on
// it, so it must change whenever the projection does.
export function layerKey(layers: LayerVisibility): string {
  return `${layers.l0 ? "1" : "0"}${layers.l1 ? "1" : "0"}`
}
...
// Project the full graph down to the nodes + links visible for `layers`. Pure: it
// reads `data` and returns new arrays, never mutating the input.
export function projectGraph(data: GraphData, layers: LayerVisibility): {
  nodes: GraphNode[]
  links: GraphLink[]
} {
  if (layers.l0 && layers.l1) {
  ...
  // L1 only: every L1 node, plus the L0 nodes anchored to one through a
