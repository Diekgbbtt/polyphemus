...
def parse_arjun(stdout: str) -> list[AssetDelta]:
    ...
    for url, url_data in arjun_output.items():
        ...
        # arjun's parameters come from the tool's own `params` list, not from
        # the key URL's query string, so only keep the BaseURL+Endpoint
        # deltas from the shared helper and emit Parameters ourselves below.
        base_deltas = url_to_deltas(url, method=method, source="arjun")[:2]
        deltas.extend(base_deltas)
        endpoint_identity = next(
            d.identity for d in base_deltas if d.type == "Endpoint"
        )
...
def parse_ffuf(stdout: str) -> list[AssetDelta]:
    ...
    for entry in results:
        ...
        # ffuf never derives Parameters from the URL's own query string -
        # only keep the BaseURL+Endpoint deltas from the shared helper.
        deltas.extend(
            url_to_deltas(
                url,
                method="GET",
                source="ffuf",
                extra_endpoint_props={"status_code": entry.get("status", 0)},
            )[:2]
        )
...
def _kiterunner_json_line(data: dict) -> list[AssetDelta] | None:
    ...
    return _kiterunner_endpoint_deltas(target, path, method, status)


def _kiterunner_endpoint_deltas(
    target: str, path: str, method: str, status: object
) -> list[AssetDelta]:
    # kiterunner never derives Parameters from a query string (path is a
    # brute-forced route, not a URL with a query) - only keep the
    # BaseURL+Endpoint deltas from the shared helper.
    url = f"{target}{path}"
