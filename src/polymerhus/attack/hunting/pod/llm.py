...
class PodHarnessContext:
    ...
    (kb_fn), the pod memory store + its spec key (spec_id = the #164 hunter's
    `<fault>_<strategy>` memory key - D84-34 - NOT the session hash), the D6 log,
    the CURRENT variant_ref for dedup scope, and the session model factory
    (None = the role's real model)."""
