...
deletes, mis-type relabels, and prop re-homing), executes them
...
from polymerhus.analysis import anatomy, index_card, l1_curator, l1_inventory, sweep
...
# `SYSTEM_EDGE_RELS` and every target `kind` is a known system kind
# (l1_curator validates both).
#
# Each rule returns (rel, kind, edge_kwargs, system_props):
#   * `rel` / `kind` - the typed edge and its target System kind;
...
def _curation_prompt(context: dict) -> str:
    ...
    return (
        "CURATION PASS. Reconcile the accumulated Layer-1 graph below.\n"
        "DEDUP IS SEMANTIC EQUIVALENCE, not key matching. Two services are the "
        "same when - reading their business_function description, exposure, data "
        "items and edges TOGETHER - they denote the SAME real-world business "
        "function, even under a different slug, singular/plural, or synonym "
        "(account == account-management; loyalty == reward-points; "
        "sign-in == signin == login). The slug is a LABEL, not the identity test. "
        "Two systems are the same when they are the same cross-cutting mechanism "
        "instance. Compare MEANINGS, then pick the better-formed key as `canonical`.\n"
        "PRECISION GUARD: genuinely distinct adjacent units must NOT be merged - "
        "cart and checkout are different functions; a CDN:Cloudflare and a "
        "CDN:Datadome are distinct instances. Merge on shared meaning, never on a "
        "shared word - a merge with no evidence is a defect.\n"
        "BEFORE returning an empty `merges` list you MUST compare every pair of "
        "services (and every pair of systems) for semantic overlap and briefly "
        "justify why each near-pair is kept distinct; an empty result is honest "
        "ONLY after that pairwise check. A different slug is NOT sufficient grounds "
        "to keep two units apart. Balance recall AND precision: propose every real "
        "duplicate.\n"
        "Propose ONLY against the keys present here; invent no node not present.\n"
        'WORKED EXAMPLE - merges: [{"kind":"service","canonical":"account",'
        '"duplicate":"account-management"},{"kind":"service","canonical":"loyalty",'
        '"duplicate":"reward-points"}] (both slugs in each pair describe the same '
        "function; keep the better-formed key).\n\n"
        "CURRENT L1 INVENTORY (propose merges / relabels against THESE exact keys):\n"
        f"  Services (business_function_slug): {inv.get('services')}\n"
        f"  Systems (kind[:discriminator]): {inv.get('systems')}\n"
        ...
        "Propose `merges`, `deletes`, `relabels`, and `rehome`."
    )
...
def default_propose_fn(context: dict) -> CurationBatch:
    ...
    structured output via function_calling (tolerates the open-ended proposal
...
# --- Stage 4: webpage anatomy (self-contained; authz deferred) ----------------
...
# --- Stage 5: FR-TYPESEP-b re-homing (deterministic backstop + LLM proposals) --
...
def _rehome_one(project_id: str, slug: str, prop_key: str, value, provenance: Provenance, seen: set) -> bool:
    ...
    if rule is None:
        return False
    rel, kind, edge_kwargs, system_props = rule(value)
    if not kind:
        return False
    seen.add(key)
    try:
        ...
        if system_props:
            l1_curator.l1_curate([], [
                SystemDelta(kind=kind, props=system_props, provenance=provenance),
            ], project_id)
        l1_curator.enrich(project_id, system_edges=[
            SystemEdgeDelta(service_slug=slug, kind=kind, rel=rel,
...
def run_curation(project_id: str, run_id: str, *, read_fn=None, propose_fn=None) -> CurationReport:
    ...
    try:
        ...
        if merges or deletes or relabels:
            ...
            # now stale. Every later stage consumes `context["cards"]`, and stage 4
    ...
    # Stage 4: run the webpage-anatomy skill for services that warrant it.
    try:
    ...
    # Stage 5: re-home System facts stranded as Service props (FR-TYPESEP-b).
    try:
    ...
    # Stage 6: the end-of-phase sweeps.
    try:
    ...
    try:
        # Redesigned missing-systems sweep (operator correction 2026-07-20): the
        # stale-L0-asset-driven ownership pass. Seam only - it proposes owners; it
        # does not yet write them back (see sweep.resolve_stale_owners' NOTE). Fail-
        # open: an LLM/read error inside it degrades to an empty batch.
        batch = sweep.resolve_stale_owners(project_id, read_fn=read_fn)
        report.stale_owners = [p.model_dump() for p in batch.proposals]
    except Exception as exc:
        logger.warning("curation.stale-owners sweep failed; fail-open", exc_info=True)
        _note_error(report, f"stale_owner_sweep: {exc}")
