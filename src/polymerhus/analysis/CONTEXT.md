...
Since #74 it carries **payloads, not cursors** (RE-SHAPED 2026-08-03): an `L0Chunk` is the job's EXACTLY-curated L0 slice - the merged `AssetDelta`s and triager `Observation`s a recon job's pods actually merged - so the consumer judges precisely what was pushed, and the feed re-reads no graph. Every pushed chunk is consumed EXACTLY ONCE, in push order: the `QueuedAnalysisFeed` is an unbounded in-memory FIFO (one consumer task per run, one chunk per pass, holding the process-wide semaphore of ONE pass - the memory bound that made a concurrent consumer admissible at all, the NM-7 ledger having rejected an unbounded one). No coalescing survives, and no cross-pass dedup state: `analyse_chunked`'s `analysed_keys` cumulative filter is GONE, and the cumulative-equals-batch guarantee (L1D-23) is preserved as the UNION over pushes - every pushed asset is judged in exactly the pass that consumes its chunk.
