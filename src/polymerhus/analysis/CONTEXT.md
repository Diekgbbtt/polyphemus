...
Mode is gated: `queued` requires BOTH `streaming_analysis` AND `async_analysis_consumer`; either off yields `inline`, today's blocking behaviour byte-for-byte (the rollback path).
