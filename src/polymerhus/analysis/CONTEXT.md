...
`push` is non-blocking (best-effort: a failing pass degrades that chunk, never the run); `drain` enqueues a TERMINAL marker chunk (empty payload, `terminal=True`) that FIFO order guarantees is consumed last, then waits bounded `deadline` + `grace` before `stop()`, and decides `analysis_drained` from that pass's `PassCensus` (`terminal` AND `unprocessed_after == 0` AND some pass entered dispatches), never from the fact a coroutine returned, because every layer beneath is fail-open. A caller without a chunk (the legacy batch path, the pod seam) keeps reading the settled surface through `run_analyser_chunked`, the batch adapter that builds one pseudo-job `L0Chunk` and hands it to the same `analyse_chunked`.
