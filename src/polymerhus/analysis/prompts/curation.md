...
(When an empty result is honest is earned under Dedup below, never assumed.)
...
In every case the stale Service prop is stripped - the outcome the analyser should have produced with a `system_edges` entry in the first place.
