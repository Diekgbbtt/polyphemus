...
Widen to everything the text supports AND ground every proposal in a specific span of it - a broad skeleton of ungrounded guesses fails exactly like a narrow one that drops half the architecture.
...
A later agent matches observed endpoints against it - it reads concrete paths (`/api/volumes/{id}`, `/account/coupons`) and matches their nouns and actions against your words.
